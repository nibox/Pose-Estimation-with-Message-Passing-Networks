...
def main():
    ...
    with torch.no_grad():
        for i in tqdm(range(len(eval_set))):
            ...
            t1 = time.time()
            ann = perd_to_ann(scoremaps[0], preds_tags, joint_det, joint_scores, img_info, int(eval_set.img_ids[i]), config.DATASET.SCALING_TYPE, True, preds_classes,
                              True, tags[0])
            t2 = time.time()
...
def perd_to_ann(scoremaps, tags, joint_det, joint_scores, img_info, img_id,
                scaling_type, adjustment, preds_classes, with_refine, tagmap):
    class Params(object):
        def __init__(self):
            self.num_joints = 17
    ...
    if len(ans) == 0:
        return None
    persons_pred_orig = reverse_affine_map(ans.copy(), (img_info["width"], img_info["height"]), scaling_type=scaling_type,
                                           min_scale=1)
