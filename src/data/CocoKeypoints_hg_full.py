...
class CocoKeypoints(Dataset):
    ...
    def __init__(self, path, mini=False, seed=0, filter_empty=True,
        ...
        ann_path_1 = f"{self.root_path}/annotations/person_keypoints_train2014.json"
        ann_path_2 = f"{self.root_path}/annotations/person_keypoints_val2014.json"
        self.coco_1 = COCO(ann_path_1)
        self.coco_2 = COCO(ann_path_2)
        ...
        self.data_dir =["train2014", "val2014"]

        self.cat_ids_1 = self.coco_1.getCatIds(catNms=["person"])
        self.img_ids = img_ids if img_ids is not None else self.coco_1.getImgIds(catIds=self.cat_ids_1)
        self.cat_ids_1 = self.coco_1.getCatIds(catNms=["person"])
        self.img_ids = img_ids if img_ids is not None else self.coco_1.getImgIds(catIds=self.cat_ids_1)
        assert len(self.img_ids) == len(set(self.img_ids))
        # process train set
        if filter_empty and img_ids is None:
            filtered_ids_fname = f"tmp/usable_ids_joint_set_1.p"
    ...
    def __getitem__(self, idx):
        ...
        return img, heatmaps, mask, keypoint_list[-1], factors, None
