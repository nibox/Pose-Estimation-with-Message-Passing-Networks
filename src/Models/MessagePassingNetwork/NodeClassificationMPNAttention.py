...
class NodeMlpType(torch.nn.Module):
    ...
    def forward(self, x, node_types):
        out = torch.zeros(x.shape[0], 17, self.output_dim, dtype=torch.float32, device=x.device)
        for i in range(17):
            out[:, i] = self.mlp_list[i](x)
        return out, out[np.arange(x.shape[0]), node_types]

class NodeClassificationMPNAttention(torch.nn.Module):
    ...
    def forward(self, x, edge_attr, edge_index, **kwargs):
        ...
        node_features_bank, node_features = self.node_embedding(x, kwargs["node_types"])
        ...
        for i in range(self.edge_steps):
            ...
            cls_features = self.classification(node_features)
            node_attention = cls_features.softmax(dim=1)
            preds_class.append(cls_features)
            node_features_initial = node_features_bank * node_attention[:, :, None]  # bank: (N, 17, dim), att: N, 17
            node_features_initial = node_features_initial.sum(dim=1)
