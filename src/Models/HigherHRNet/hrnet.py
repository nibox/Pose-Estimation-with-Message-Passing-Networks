...
class PoseHigherResolutionNet(nn.Module):
    ...
    def __init__(self, cfg, **kwargs):
        ...
        self.stage2_cfg = cfg['MODEL']["HRNET"]['EXTRA']['STAGE2']
        ...
        self.stage3_cfg = cfg['MODEL']["HRNET"]['EXTRA']['STAGE3']
        ...
        self.stage4_cfg = cfg['MODEL']["HRNET"]['EXTRA']['STAGE4']
        ...
        self.deconv_config = cfg.MODEL.HRNET.EXTRA.DECONV
        self.loss_config = cfg.MODEL.HRNET.LOSS

        self.pretrained_layers = cfg['MODEL']["HRNET"]['EXTRA']['PRETRAINED_LAYERS']

    def _make_final_layers(self, cfg, input_channels):
        dim_tag = cfg.MODEL.HRNET.NUM_JOINTS if cfg.MODEL.HRNET.TAG_PER_JOINT else 1
        extra = cfg.MODEL.HRNET.EXTRA

        final_layers = []
        output_channels = cfg.MODEL.HRNET.NUM_JOINTS + dim_tag \
            if cfg.MODEL.HRNET.LOSS.WITH_AE_LOSS[0] else cfg.MODEL.HRNET.NUM_JOINTS
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            ...
            output_channels = cfg.MODEL.NUM_JOINTS + dim_tag \
                if cfg.MODEL.HRNET.LOSS.WITH_AE_LOSS[i+1] else cfg.MODEL.HRNET.NUM_JOINTS
            final_layers.append(nn.Conv2d(
                in_channels=input_channels,
                out_channels=output_channels,
                kernel_size=extra.FINAL_CONV_KERNEL,
    ...
    def _make_deconv_layers(self, cfg, input_channels):
        ...
        for i in range(deconv_cfg.NUM_DECONVS):
            if deconv_cfg.CAT_OUTPUT[i]:
                final_output_channels = cfg.MODEL.HRNET.NUM_JOINTS + dim_tag \
                    if cfg.MODEL.HRNET.LOSS.WITH_AE_LOSS[i] else cfg.MODEL.HRNET.NUM_JOINTS
            ...
            for _ in range(cfg.MODEL.HRNET.EXTRA.DECONV.NUM_BASIC_BLOCKS):
                layers.append(nn.Sequential(
