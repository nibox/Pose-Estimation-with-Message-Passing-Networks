...
def main():
    ...
    with torch.no_grad():
        ...
        for i in tqdm(range(num_iter)):
            ...
            ann_oracle = perd_to_ann(scoremaps[0], tags[0], joint_det, preds_nodes, edge_index, edge_labels,
                                              img_info, int(eval_set.img_ids[i]), config.MODEL.GC.CC_METHOD,
                                              scaling_type, min(config.TEST.SCALE_FACTOR), True,
                                              config.MODEL.MPN.NODE_THRESHOLD, preds_classes_gt, False, joint_scores, False)
        ...
        eval_writer.eval_coco(eval_set.coco, anns_oracle, np.array(eval_ids), "Perfect Edges and classes", "full_dt.json")
